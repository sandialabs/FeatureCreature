'''
This is the working class for Chemofeature collection
'''
...
class TestingUpdate(object):
    ...
    def update(self, padel=False):
        if padel is False:
            ...
            #     "-threads", "-1"])
            call(["java", "-jar", _directory+"/PaDEL-Descriptor.jar", "-threads", "-1",
                  "-dir", _testing_directory, "-file", _padel_descriptor, "-fingerprints",
                  "-retainorder", "-usefilenameasmolname", "-descriptortypes",
                  _directory+"/descriptors.xml"])
        for cid in cid_list:
            filename = _testing_directory + str(cid) + ".sdf"
            os.remove(filename)
        self._parsePadel()
...
class Update(object):
    ...
    def _variable_compounds(self, testing):
        if testing and not self.private:
        	dirpath = _testing_directory
        elif self.private:
            dirpath = self.private
        else:
        ...
        if self.padel is False:
            ...
            #     "-threads", "-1"])
            call(["java", "-jar", _directory+"/PaDEL-Descriptor.jar", "-threads", "-1",
                  "-dir", dirpath, "-file", _padel_descriptor, "-fingerprints",
                  "-retainorder", "-usefilenameasmolname", "-descriptortypes",
                  _directory+"/descriptors.xml"])
            self._parsePadel()
    ...
    def update(self, padel=True, clean=False, private=False):
        self.padel = padel  # Run PaDEL-Descriptor
        self.clean = clean  # Clean working directory
        self.private = private
        self._variable_compounds(testing=self.testing)
        if self.remove is True:
            self._update_compounds()
        del self.variable
        del self.total

    def __init__(self, compounds, verbose=False, remove_static=True, testing=False):
