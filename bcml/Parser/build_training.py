...
from bcml.Boruta import boruta_py
...
def _get_feature_names(compounds):
    ...
    for compound in compounds:
        for feature in sorted(_possible_features):
            if 'experimentalhash' in compound.keys() and feature == 'experimentalhash':
...
class Process(object):
    ...
    def __init__(self, model_input, split_value=False, verbose=False):
        ...
        self.predictor_values = np.array(predictors, '|S4').astype(np.float)
        if split_value:
            self.predictors = _convert_predictor(self.predictor_values, split_value)
        else:
            print_line = "Splitting at " + str(np.median(self.predictor_values))
            verbose_print(verbose, print_line)
            self.predictors = _convert_predictor(self.predictor_values,
                                                 np.median(self.predictor_values))
        ...
        np.savetxt("testing_data.csv", self.train, delimiter=",")
