...
class LimeTabularExplainer(object):
    ...
    def __init__(self, training_data, training_labels=None, feature_names=None,
        ...
        self.discretized_training_data = discretized_training_data
    ...
    def __get_inverse(self, data_row, num_samples, num_features):
        ...
        for i, row in enumerate(inverse):
            random_row = np.random.choice(len(self.discretized_training_data), size=1, replace=True)
            array = np.random.random(size=num_features) < distance.cosine(row, self.discretized_training_data[random_row,:])
            np.putmask(row, array, self.discretized_training_data[random_row,:])
            inverse[i,:] = row
            binary_row = np.array([1 if x == first_row[column]
                                   else 0 for x in inverse[i, :]])
            binary_row[0] = 1
            data[i, :] = binary_row
    ...
    def explain_instance(self, data_row, classifier_fn, labels=(1,),
                         ...
                         distance_metric='euclidean', model_regressor=None, verbose=False):
        ...
        Returns:
            ...
            explanations.
        """
        #data, inverse = self.__get_inverse(data_row, num_samples, num_features)
    ...
    def __data_inverse(self,
        ...
        for column in categorical_features:
            values = self.feature_values[column]
            freqs = self.feature_frequencies[column]
            #sample = [stats.gamma.rvs(a, 1., 10.) for a in freqs]
            #sample = [v/sum(sample) for v in sample]
            inverse_column = np.random.choice(values, size=num_samples,
                                              replace=True, p=freqs)
            #array  = np.random.random(size=num_samples) <= 1.-(np.mean((1.-freqs)*freqs))
            #np.putmask(inverse_column, array, data_row[column])
